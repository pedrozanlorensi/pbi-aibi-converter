...
# Mapping moved to `widget_catalog.py` (shared with converter.py).
    ...
    for pbi_page_idx, pbi_page in enumerate(pbi_layout.pages):
        ...
        for pbi_vis in pbi_non_text:
            ...
            if best_match is not None:
                ...
                pos = item.get("position", {})
                # Track drift in BOTH dimensions. Previously only x and
                # width were checked, so a widget that landed 10 grid
                # rows below its source position never raised a warning
                # — the layout fidelity report read green even when the
                # dashboard was visibly wrong. Thresholds:
                #   * x  : > 1 col   (one column of slack is fine)
                #   * w  : > 2 cols  (sizes drift more than positions)
                #   * y  : > 2 rows  (single-row settle from collision
                #                     resolution is OK)
                #   * h  : > 2 rows
                x_drift = abs(pos.get("x", 0) - pbi_vis.grid_x)
                y_drift = abs(pos.get("y", 0) - pbi_vis.grid_y)
                w_drift = abs(pos.get("width", 1) - pbi_vis.grid_width)
                h_drift = abs(pos.get("height", 1) - pbi_vis.grid_height)
                if (x_drift > 1 or w_drift > 2
                        or y_drift > 2 or h_drift > 2):
                    result.position_warnings.append({
                        ...
                        "expected_y": pbi_vis.grid_y,
                        "expected_w": pbi_vis.grid_width,
                        "expected_h": pbi_vis.grid_height,
                        "actual_x": pos.get("x", "?"),
                        "actual_y": pos.get("y", "?"),
                        "actual_w": pos.get("width", "?"),
                        "actual_h": pos.get("height", "?"),
                    })
            else:
