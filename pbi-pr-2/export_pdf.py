...
def build_export_pdf(
    ...
    if lf:
        ...
        for entry in lf.page_visual_counts or []:
            ...
            if actual == expected:
                ...
                # More widgets than the source had — usually accepted
                # alternatives that didn't replace the LLM's main-pass
                # widget. Worth flagging.
                status = "WARN"
            item(status, f"{entry['name']}: {actual} widgets (expected {expected})")
    ...
    # ---- Structural Checks ----
    # `validation.errors` is a single bag containing widget-version
    # mismatches, fieldName/encoding misalignment, dataset reference
    # errors, AND SQL execution failures. We can't honestly assert
    # which of those individual checks passed without finer-grained
    # categorization in `validation`, so we just summarize.
    heading("Structural Checks")
    if not validation.errors:
        item("PASS", "No structural errors found")
    else:
